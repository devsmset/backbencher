      ...
      <Panel title="API calls">
        ...
        {calls.map((c) => (
          <details key={c.correlationId || `${c.ts}-${c.url}`} className="mb-2 overflow-hidden rounded-md border border-[--line]">
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-[--line] p-3 max-[900px]:grid-cols-1">
              <div>
                <h4>Request</h4>
                <pre className="max-h-[220px] overflow-auto rounded-[10px] border border-[--line] bg-[#0a1016] p-3 font-mono text-xs leading-[1.55]">
                  {JSON.stringify({ headers: c.reqHeaders, body: c.reqBody }, null, 2)}
                </pre>
              </div>
              <div>
                <h4>Response</h4>
                <pre className="max-h-[220px] overflow-auto rounded-[10px] border border-[--line] bg-[#0a1016] p-3 font-mono text-xs leading-[1.55]">
                  {JSON.stringify({ bodyKind: c.bodyKind, headers: c.resHeaders, body: c.resBody }, null, 2)}
                </pre>
              </div>
            </div>
          </details>
